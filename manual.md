...
shutdown,"Alias for exit. Terminates the shell, signaling the kernel for shutdown (simulated).",SYSCALL_TASK_EXIT(0)
cd <path>,Changes the current working directory (CWD) of the shell.,SYSCALL_RESOURCE_ACQUIRE (validation)
...
sleep <ms>,Pauses the execution of the shell for the specified duration in milliseconds.,SYSCALL_TASK_SLEEP
...
irrecoverable_deletion,"Alias for delete, emphasizing the permanent nature of the action.",Same as delete
...
Ctrl + X,Cut,Editor,Copies the command line content to the internal clipboard and clears the line.
Ctrl + V,Paste,Editor,Pastes content from the internal clipboard to the cursor position.
...
Ctrl + A,Select All,Editor,"Placeholder: Currently implemented as ""Go to start of line"" or simple select."
Ctrl + S,Save,Built-in,Executes the save command.
Ctrl + Shift + L,Insert Separator,Custom,Inserts the custom path separator character: ∣ (Pipe Symbol).
